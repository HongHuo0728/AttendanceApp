...
bool ThemedConfirm(const std::wstring& title, const std::wstring& message, HWND owner = nullptr);
void ThemedMessage(const std::wstring& title, const std::wstring& message, HWND owner = nullptr);
...
LRESULT CALLBACK ConfirmDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    auto* state = reinterpret_cast<ConfirmDialogState*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    switch (msg) {
    case WM_CREATE: {
        auto* create = reinterpret_cast<CREATESTRUCTW*>(lParam);
        state = reinterpret_cast<ConfirmDialogState*>(create->lpCreateParams);
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, (LONG_PTR)state);
        MakeSettingsControl(hwnd, L"STATIC", state->message.c_str(), SS_LEFT, 0);
        std::wstring yesText = Tr(L"OK", L"\u786e\u5b9a");
        std::wstring noText = Tr(L"Cancel", L"\u53d6\u6d88");
        MakeSettingsControl(hwnd, L"BUTTON", yesText.c_str(), BS_PUSHBUTTON | BS_OWNERDRAW | WS_TABSTOP, IDC_INPUT_OK);
        if (state->confirm) {
            MakeSettingsControl(hwnd, L"BUTTON", noText.c_str(), BS_PUSHBUTTON | BS_OWNERDRAW | WS_TABSTOP, IDC_INPUT_CANCEL);
        }
        ApplyThemedControls(hwnd);
        SetFocus(GetDlgItem(hwnd, IDC_INPUT_OK));
        return 0;
    }
    case WM_SIZE: {
        RECT rc{};
        ...
        HWND message = FindWindowExW(hwnd, nullptr, L"STATIC", nullptr);
        MoveWindow(message, 24, 24, w - 48, h - 92, TRUE);
        if (state && state->confirm) {
            MoveWindow(GetDlgItem(hwnd, IDC_INPUT_OK), w - 204, h - 52, 82, 34, TRUE);
            MoveWindow(GetDlgItem(hwnd, IDC_INPUT_CANCEL), w - 110, h - 52, 86, 34, TRUE);
        } else {
            MoveWindow(GetDlgItem(hwnd, IDC_INPUT_OK), w - 114, h - 52, 90, 34, TRUE);
        }
        return 0;
    }
    case WM_COMMAND:
    ...
    case WM_KEYDOWN:
        ...
        if (wParam == VK_ESCAPE) {
            SendMessageW(hwnd, WM_COMMAND, IDC_INPUT_CANCEL, 0);
            return 0;
        }
        break;
    case WM_ERASEBKGND: {
...
bool ThemedConfirm(const std::wstring& title, const std::wstring& message, HWND owner) {
    if (!owner) owner = g_hwnd;
    HINSTANCE instance = GetModuleHandleW(nullptr);
    const wchar_t* className = L"AttendanceConfirmDialog";
    ...
    ConfirmDialogState state{title, message, true, false};
...
void ThemedMessage(const std::wstring& title, const std::wstring& message, HWND owner) {
    if (!owner) owner = g_hwnd;
    HINSTANCE instance = GetModuleHandleW(nullptr);
    const wchar_t* className = L"AttendanceConfirmDialog";
    static bool registered = false;
    ...
    ConfirmDialogState state{title, message, false, false};
    if (owner) EnableWindow(owner, FALSE);
    HWND dialog = CreateWindowExW(
        WS_EX_DLGMODALFRAME,
        className,
...
bool ConfirmDiscardUnsaved(const std::wstring& actionText) {
    ...
    return ThemedConfirm(title, msg);
}
...
bool TranslateAdditionalLanguage(const std::wstring& key, std::wstring& out) {
    if (g_language == UiLanguage::Norwegian || g_language == UiLanguage::Vietnamese || g_language == UiLanguage::ChineseTraditionalHongKong) {
        ...
        static const V12Entry entries[] = {
            ...
            {L"Unsaved Changes", L"Ulagrede endringer", L"Thay \u0111\u1ed5i ch\u01b0a l\u01b0u", L"\u672a\u5132\u5b58\u66f4\u6539"},
            {L"You have unsaved changes. Continue and discard them?", L"Du har ulagrede endringer. Fortsette og forkaste dem?", L"B\u1ea1n c\u00f3 thay \u0111\u1ed5i ch\u01b0a l\u01b0u. Ti\u1ebfp t\u1ee5c v\u00e0 b\u1ecf qua?", L"\u6709\u672a\u5132\u5b58\u66f4\u6539\u3002\u7e7c\u7e8c\u4e26\u653e\u68c4\uff1f"},
            {L"Reset Settings", L"Tilbakestill innstillinger", L"\u0110\u1eb7t l\u1ea1i c\u00e0i \u0111\u1eb7t", L"\u91cd\u8a2d\u8a2d\u5b9a"},
            {L"Reset all settings and delete the AppData configuration file?", L"Tilbakestille alle innstillinger og slette AppData-konfigurasjonen?", L"\u0110\u1eb7t l\u1ea1i t\u1ea5t c\u1ea3 c\u00e0i \u0111\u1eb7t v\u00e0 x\u00f3a t\u1ec7p c\u1ea5u h\u00ecnh AppData?", L"\u91cd\u8a2d\u6240\u6709\u8a2d\u5b9a\u4e26\u522a\u9664 AppData \u8a2d\u5b9a\u6a94\uff1f"},
    ...
    static const Entry entries[] = {
        ...
        {L"Unsaved Changes", L"Modifiche non salvate", L"\u0425\u0430\u0434\u0433\u0430\u043b\u0430\u0430\u0433\u04af\u0439 \u04e9\u04e9\u0440\u0447\u043b\u04e9\u043b\u0442", L"Nekonservitaj \u015dan\u011doj", L"\u672a\u85cf\u4e4b\u6539", L"\u0e01\u0e32\u0e23\u0e40\u0e1b\u0e25\u0e35\u0e48\u0e22\u0e19\u0e41\u0e1b\u0e25\u0e07\u0e17\u0e35\u0e48\u0e22\u0e31\u0e07\u0e44\u0e21\u0e48\u0e1a\u0e31\u0e19\u0e17\u0e36\u0e01", L"Hindi pa nase-save", L"Kaydedilmemi\u015f de\u011fi\u015fiklikler", L"Nei\u0161saugoti pakeitimai"},
        {L"You have unsaved changes. Continue and discard them?", L"Hai modifiche non salvate. Continuare e scartarle?", L"\u0422\u0430\u043d\u0434 \u0445\u0430\u0434\u0433\u0430\u043b\u0430\u0430\u0433\u04af\u0439 \u04e9\u04e9\u0440\u0447\u043b\u04e9\u043b\u0442 \u0431\u0430\u0439\u043d\u0430. \u04ae\u0440\u0433\u044d\u043b\u0436\u043b\u04af\u04af\u043b\u044d\u044d\u0434 \u0445\u0430\u044f\u0445 \u0443\u0443?", L"Vi havas nekonservitajn \u015dan\u011dojn. \u0108u da\u016drigi kaj forĵeti ilin?", L"\u6709\u672a\u85cf\u4e4b\u6539\u3002\u4ecd\u884c\u800c\u68c4\u4e4b\u4e4e\uff1f", L"\u0e21\u0e35\u0e01\u0e32\u0e23\u0e40\u0e1b\u0e25\u0e35\u0e48\u0e22\u0e19\u0e41\u0e1b\u0e25\u0e07\u0e17\u0e35\u0e48\u0e22\u0e31\u0e07\u0e44\u0e21\u0e48\u0e1a\u0e31\u0e19\u0e17\u0e36\u0e01 \u0e08\u0e30\u0e14\u0e33\u0e40\u0e19\u0e34\u0e19\u0e15\u0e48\u0e2d\u0e41\u0e25\u0e30\u0e17\u0e34\u0e49\u0e07\u0e44\u0e1b\u0e2b\u0e23\u0e37\u0e2d\u0e44\u0e21\u0e48?", L"May hindi pa nase-save. Magpatuloy at itapon ang mga ito?", L"Kaydedilmemi\u015f de\u011fi\u015fiklikler var. Devam edip vazge\u00e7ilsin mi?", L"Yra nei\u0161saugot\u0173 pakeitim\u0173. T\u0119sti ir juos atmesti?"},
        {L"Reset Settings", L"Ripristina impostazioni", L"\u0422\u043e\u0445\u0438\u0440\u0433\u043e\u043e\u0433 \u0441\u044d\u0440\u0433\u044d\u044d\u0445", L"Restarigi agordojn", L"\u5fa9\u8a2d", L"\u0e23\u0e35\u0e40\u0e0b\u0e47\u0e15\u0e01\u0e32\u0e23\u0e15\u0e31\u0e49\u0e07\u0e04\u0e48\u0e32", L"I-reset ang settings", L"Ayarlar\u0131 s\u0131f\u0131rla", L"Atkurti nustatymus"},
        {L"Reset all settings and delete the AppData configuration file?", L"Ripristinare tutte le impostazioni ed eliminare il file AppData?", L"\u0411\u04af\u0445 \u0442\u043e\u0445\u0438\u0440\u0433\u043e\u043e\u0433 \u0441\u044d\u0440\u0433\u044d\u044d\u0433\u044d\u044d\u0434 AppData \u0442\u043e\u0445\u0438\u0440\u0433\u043e\u043e\u043d\u044b \u0444\u0430\u0439\u043b\u044b\u0433 \u0443\u0441\u0442\u0433\u0430\u0445 \u0443\u0443?", L"\u0108u restarigi \u0109iujn agordojn kaj forigi la AppData-dosieron?", L"\u6b32\u5fa9\u8af8\u8a2d\u800c\u522a AppData \u8a2d\u6a94\u4e4e\uff1f", L"\u0e23\u0e35\u0e40\u0e0b\u0e47\u0e15\u0e01\u0e32\u0e23\u0e15\u0e31\u0e49\u0e07\u0e04\u0e48\u0e32\u0e17\u0e31\u0e49\u0e07\u0e2b\u0e21\u0e14\u0e41\u0e25\u0e30\u0e25\u0e1a\u0e44\u0e1f\u0e25\u0e4c AppData?", L"I-reset lahat ng settings at burahin ang AppData config file?", L"T\u00fcm ayarlar s\u0131f\u0131rlans\u0131n ve AppData yap\u0131land\u0131rma dosyas\u0131 silinsin mi?", L"Atkurti visus nustatymus ir i\u0161trinti AppData konfig\u016bracijos fail\u0105?"},
...
std::wstring Tr(const wchar_t* english, const wchar_t*) {
    ...
    static const Entry entries[] = {
        ...
        {L"Reset Settings", L"\u91cd\u7f6e\u8bbe\u7f6e", L"Irrisettja settings", L"\u8a2d\u5b9a\u3092\u30ea\u30bb\u30c3\u30c8", L"R\u00e9initialiser les param\u00e8tres", L"Einstellungen zur\u00fccksetzen", L"\u0421\u0431\u0440\u043e\u0441 \u043d\u0430\u0441\u0442\u0440\u043e\u0435\u043a", L"\u91cd\u8a2d\u8a2d\u5b9a", L"Restablecer configuraci\u00f3n"},
        {L"Reset all settings and delete the AppData configuration file?", L"\u91cd\u7f6e\u6240\u6709\u8bbe\u7f6e\u5e76\u5220\u9664 AppData \u914d\u7f6e\u6587\u4ef6\uff1f", L"Irrisettja kollox u \u0127assar il-fajl AppData?", L"\u3059\u3079\u3066\u306e\u8a2d\u5b9a\u3092\u30ea\u30bb\u30c3\u30c8\u3057\u3066 AppData \u8a2d\u5b9a\u30d5\u30a1\u30a4\u30eb\u3092\u524a\u9664\u3057\u307e\u3059\u304b\uff1f", L"R\u00e9initialiser tous les param\u00e8tres et supprimer le fichier AppData ?", L"Alle Einstellungen zur\u00fccksetzen und die AppData-Konfigurationsdatei l\u00f6schen?", L"\u0421\u0431\u0440\u043e\u0441\u0438\u0442\u044c \u0432\u0441\u0435 \u043d\u0430\u0441\u0442\u0440\u043e\u0439\u043a\u0438 \u0438 \u0443\u0434\u0430\u043b\u0438\u0442\u044c \u0444\u0430\u0439\u043b AppData?", L"\u91cd\u8a2d\u6240\u6709\u8a2d\u5b9a\u4e26\u522a\u9664 AppData \u914d\u7f6e\u6a94\uff1f", L"\u00bfRestablecer todo y borrar el archivo AppData?"},
        {L"Unsaved Changes", L"\u672a\u4fdd\u5b58\u7684\u4fee\u6539", L"Bidliet mhux salvati", L"\u672a\u4fdd\u5b58\u306e\u5909\u66f4", L"Modifications non enregistr\u00e9es", L"Ungespeicherte \u00c4nderungen", L"\u041d\u0435\u0441\u043e\u0445\u0440\u0430\u043d\u0451\u043d\u043d\u044b\u0435 \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u044f", L"\u672a\u5132\u5b58\u7684\u4fee\u6539", L"Cambios sin guardar"},
        {L"You have unsaved changes. Continue and discard them?", L"\u5f53\u524d\u6709\u672a\u4fdd\u5b58\u7684\u4fee\u6539\u3002\u662f\u5426\u7ee7\u7eed\u5e76\u653e\u5f03\u8fd9\u4e9b\u4fee\u6539\uff1f", L"G\u0127andek bidliet mhux salvati. Tkompli u twarrabhom?", L"\u672a\u4fdd\u5b58\u306e\u5909\u66f4\u304c\u3042\u308a\u307e\u3059\u3002\u7d9a\u884c\u3057\u3066\u7834\u68c4\u3057\u307e\u3059\u304b\uff1f", L"Des modifications ne sont pas enregistr\u00e9es. Continuer et les abandonner ?", L"Es gibt ungespeicherte \u00c4nderungen. Fortfahren und verwerfen?", L"\u0415\u0441\u0442\u044c \u043d\u0435\u0441\u043e\u0445\u0440\u0430\u043d\u0451\u043d\u043d\u044b\u0435 \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u044f. \u041f\u0440\u043e\u0434\u043e\u043b\u0436\u0438\u0442\u044c \u0438 \u0441\u0431\u0440\u043e\u0441\u0438\u0442\u044c?", L"\u76ee\u524d\u6709\u672a\u5132\u5b58\u7684\u4fee\u6539\u3002\u662f\u5426\u7e7c\u7e8c\u4e26\u653e\u68c4\uff1f", L"Hay cambios sin guardar. \u00bfContinuar y descartarlos?"},
...
void DeleteCurrentCourse() {
    ...
    if (!ThemedConfirm(deleteCourseTitle, deleteCourseMsg)) {
        return;
    }
    PushUndo();
    MarkDirty();
...
void MarkAllPresent() {
    ...
    if (ThemedConfirm(allPresentTitle, allPresentMsg)) {
        PushUndo();
        MarkDirty();
...
void DeleteSelectedRecords() {
    ...
    if (ThemedConfirm(deleteTitle, ss.str())) {
        EraseRows(rows);
    }
}
...
void DeleteRecordsByStatus(const std::wstring& status) {
    ...
    std::wstringstream ss;
    std::wstring statusLabel = status == L"Absent" ? Tr(L"Absent", L"\u7f3a\u5e2d") : Tr(L"Late", L"\u8fdf\u5230");
    ss << Tr(L"Delete all", L"\u5220\u9664\u6240\u6709") << L" " << statusLabel << L" "
       << Tr(L"records?", L"\u8bb0\u5f55\uff1f") << L" (" << rows.size() << L")";
    std::wstring batchTitle = Tr(L"Batch Delete", L"\u6279\u91cf\u5220\u9664");
    if (ThemedConfirm(batchTitle, ss.str())) {
        EraseRows(rows);
    }
}
...
void ClearAllRecords() {
    ...
    if (ThemedConfirm(clearTitle, clearMsg)) {
        PushUndo();
        MarkDirty();
        g_records.clear();
        RefreshList();
...
LRESULT CALLBACK SettingsProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == IDC_SETTINGS_RESET) {
            ...
            if (ThemedConfirm(resetTitle, resetMsg, hwnd)) {
                ResetSettings();
                ApplyVisualSettings();
                EnumChildWindows(hwnd, ApplyFontToChild, 0);
                SendMessageW(GetDlgItem(hwnd, IDC_SETTINGS_TITLE), WM_SETFONT, (WPARAM)g_titleFont, TRUE);
